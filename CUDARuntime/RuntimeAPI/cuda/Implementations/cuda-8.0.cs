...
namespace Hybridizer.Runtime.CUDAImports
{
    ...
    internal unsafe partial class CudaImplem
    {
        ...
        internal class Cuda_32_80 : ICuda
        {
...
#else
            internal const string CUDARTDLL = "cudart32_80.dll";
#endif
            ...
            public static extern cudaError_t cudaGetDeviceProperties(out cudaDeviceProp_80 props, int device);

            public cudaError_t GetDeviceProperties(out cudaDeviceProp props, int device)
            {
                cudaDeviceProp_80 res;
            ...
            {
                return cudaGetDeviceFlags(out flags);
            }
            ...
            public static extern cudaError_t cudaChooseDevice(out int device, ref cudaDeviceProp_80 prop);

            public cudaError_t ChooseDevice(out int device, ref cudaDeviceProp prop)
            {
                cudaDeviceProp_80 res;
                res = cuda.StructConvert<cudaDeviceProp_80>(prop);
            ...
            public static extern cudaError_t cudaDeviceGetP2PAttribute(out int value, cudaDeviceP2PAttr attr,
                int srcDevice, int dstDevice);

            public cudaError_t DeviceGetP2PAttribute(out int value, cudaDeviceP2PAttr attr, int srcDevice,
                int dstDevice)
            {
                return cudaDeviceGetP2PAttribute(out value, attr, srcDevice, dstDevice);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaDeviceGetStreamPriorityRange")]
            public static extern cudaError_t cudaDeviceGetStreamPriorityRange(out int leastPriority,
                out int greatestPriority);

            public cudaError_t DeviceGetStreamPriorityRange(out int leastPriority, out int greatestPriority)
            {
                return cudaDeviceGetStreamPriorityRange(out leastPriority, out greatestPriority);
            }
            ...
            {
                return Marshal.PtrToStringAnsi(cudaGetErrorName(err));
            }
            ...
            public static extern cudaError_t cudaStreamAddCallback(cudaStream_t stream, cudaStreamCallback_t callback,
                IntPtr userData, uint flags);

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaStreamAttachMemAsync")]
            public static extern cudaError_t cudaStreamAttachMemAsync(cudaStream_t stream, IntPtr devPtr, size_t length,
                uint flags);
            ...
            public static extern cudaError_t cudaStreamCreateWithPriority(out cudaStream_t pStream, uint flags,
                int priority);
            ...
            public cudaError_t StreamAddCallback(cudaStream_t stream, cudaStreamCallback_t callback, IntPtr userData,
                uint flags)
            {
                return StreamAddCallback(stream, callback, userData, flags);
            }
            ...
            public static extern cudaError_t cudaLaunchKernel(IntPtr func, dim3 gridDim, dim3 blockDim, IntPtr args,
            ...
            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi,
                EntryPoint = "cudaOccupancyMaxActiveBlocksPerMultiprocessor ")]
            public static extern cudaError_t cudaOccupancyMaxActiveBlocksPerMultiprocessor(out int numBlocks,
                IntPtr function, int blockSize, size_t dynamicSharedSize);

            public cudaError_t OccupancyMaxActiveBlocksPerMultiprocessor(out int numBlocks, IntPtr function,
                int blockSize, size_t dynamicSharedSize)
            {
                return cudaOccupancyMaxActiveBlocksPerMultiprocessor(out numBlocks, function, blockSize,
                    dynamicSharedSize);
            }
            ...
            static extern cudaError_t cudaArrayGetInfo(out cudaChannelFormatDesc desc, out cudaFuncAttributes extent,
                out uint flags, cudaArray_t array); // Gets info about the specified cudaArray. 
            ...
            static extern cudaError_t
                cudaFreeMipmappedArray(cudaMipmappedArray_t mipmappedArray); // Frees a mipmapped array on the device. 
            ...
            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaFreeMipmappedArray")]
            static extern cudaError_t cudaGetMipmappedArrayLevel(out cudaArray_t levelArray,
                cudaMipmappedArray_t mipmappedArray, uint level); // Gets a mipmap level of a CUDA mipmapped array. 

            public cudaError_t GetMipmappedArrayLevel(out cudaArray_t levelArray, cudaMipmappedArray_t mipmappedArray,
                uint level)
            {
                return cudaGetMipmappedArrayLevel(out levelArray, mipmappedArray, level);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMallocManaged")]
            static extern cudaError_t
                cudaMallocManaged(out IntPtr devPtr, size_t size,
                    uint flags); // Allocates memory that will be automatically managed by the Unified Memory system. 

            public cudaError_t MallocManaged(out IntPtr devPtr, size_t size, uint flags)
            {
                return cudaMallocManaged(out devPtr, size, flags);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMallocMipmappedArray")]
            static extern cudaError_t cudaMallocMipmappedArray(out cudaMipmappedArray_t mipmappedArray,
                ref cudaChannelFormatDesc desc, cudaFuncAttributes extent, int numLevels,
                uint flags); // Allocate a mipmapped array on the device. 

            public cudaError_t MallocMipmappedArray(out cudaMipmappedArray_t mipmappedArray,
                ref cudaChannelFormatDesc desc, cudaFuncAttributes extent, int numLevels, uint flags = 0)
            {
            ...
            public cudaError_t Memcpy3DPeer(ref cudaMemcpy3DPeerParms p)
            {
                return cudaMemcpy3DPeer(ref p);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemcpy3DPeerAsync")]
            static extern cudaError_t
                cudaMemcpy3DPeerAsync(ref cudaMemcpy3DPeerParms p,
                    cudaStream_t stream); // Copies memory between devices asynchronously. 

            public cudaError_t Memcpy3DPeerAsync(ref cudaMemcpy3DPeerParms p, cudaStream_t stream = new cudaStream_t())
            {
                return cudaMemcpy3DPeerAsync(ref p, stream);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemsetAsync")]
            public static extern cudaError_t cudaMemsetAsync(IntPtr devPtr, int value, size_t count,
                cudaStream_t stream);

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemset2DAsync")]
            public static extern cudaError_t cudaMemset2DAsync(IntPtr devPtr, size_t pitch, int value, size_t width,
                size_t height, cudaStream_t stream);

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemset3DAsync")]
            public static extern cudaError_t cudaMemset3DAsync(cudaPitchedPtr devPtr, int value, cudaFuncAttributes extent,
                cudaStream_t stream);

            public cudaError_t MemsetAsync(IntPtr devPtr, int value, size_t count,
            ...
            public cudaError_t Memset3DAsync(cudaPitchedPtr devPtr, int value, cudaFuncAttributes extent,
                cudaStream_t stream = new cudaStream_t())
            {
                return cudaMemset3DAsync(devPtr, value, extent, stream);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemPrefetchAsync")]
            public static extern cudaError_t cudaMemPrefetchAsync(IntPtr devptr, size_t count, int dstDevice,
                cudaStream_t stream);
            ...
            {
                return cudaMemPrefetchAsync(devptr, count, dstDevice, cudaStream_t.NO_STREAM);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemcpyPeerAsync")]
            static extern cudaError_t cudaMemcpyPeerAsync(IntPtr dest, int dstDevice, IntPtr src, int srcDevice,
                size_t count, cudaStream_t stream);

            public cudaError_t MemcpyPeerAsync(IntPtr dest, int dstDevice, IntPtr src, int srcDevice, size_t count,
                cudaStream_t stream)
            {
                return cudaMemcpyPeerAsync(dest, dstDevice, src, srcDevice, count, stream);
            }

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaMemcpyPeer")]
            static extern cudaError_t cudaMemcpyPeer(IntPtr dest, int dstDevice, IntPtr src, int srcDevice,
                size_t count);

            public cudaError_t MemcpyPeer(IntPtr dest, int dstDevice, IntPtr src, int srcDevice, size_t count)
            {
                return cudaMemcpyPeer(dest, dstDevice, src, srcDevice, count);
            }
            ...
            public cudaError_t MallocMipmappedArray(out cudaMipmappedArray_t mipmappedArray,
                ref cudaChannelFormatDesc desc, cudaFuncAttributes extent, uint numLevels, uint flags = 0)
            {
                return cudaMallocMipmappedArray(out mipmappedArray, ref desc, extent, numLevels, flags);
            }
            ...
            public static extern cudaError_t cudaCreateTextureObject(out cudaTextureObject_t texture,
                ref cudaResourceDesc resDesc, ref cudaTextureDesc texDesc, ref cudaResourceViewDesc ResViewDesc);

            [DllImport(CUDARTDLL, CharSet = CharSet.Ansi, EntryPoint = "cudaCreateTextureObject")]
            public static extern cudaError_t cudaCreateTextureObject(out cudaTextureObject_t texture,
                ref cudaResourceDesc resDesc, ref cudaTextureDesc texDesc, IntPtr ResViewDesc);
            ...
            public static extern cudaError_t cudaGetTextureObjectResourceDesc(out cudaResourceDesc resDesc,
            ...
            {
                return cudaCreateTextureObject(out texture, ref resDesc, ref texDesc, ref ResViewDesc);
            }

            public cudaError_t CreateTextureObject(out cudaTextureObject_t texture, ref cudaResourceDesc resDesc,
                ref cudaTextureDesc texDesc)
            {
                return cudaCreateTextureObject(out texture, ref resDesc, ref texDesc, IntPtr.Zero);
            }
            ...
            {
                return cudaGetTextureObjectResourceDesc(out resDesc, texture);
            }
...
#pragma warning restore 0169
            ...
            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsGLRegisterBuffer(out IntPtr pCudaResource, uint buffer,
                uint Flags);
            ...
            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsResourceGetMappedPointer(out IntPtr devPtr, out size_t size,
                IntPtr resource);
            ...
            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsMapResources(int count, IntPtr[] resources,
                cudaStream_t stream);

            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsUnmapResources(int count, IntPtr[] resources,
                cudaStream_t stream);
        ...
        internal class Cuda_64_80 : ICuda
        {
            ...
            internal const string CUDARTDLL = "libcudart.so";
#else
            internal const string CUDARTDLL = "cudart64_80.dll";
#endif
...
#pragma warning restore 0169
            ...
            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsGLRegisterImage(out IntPtr cudaGraphicsResource, uint image,
                uint target, uint flags);

            [DllImport(CUDARTDLL, CallingConvention = CallingConvention.Cdecl)]
            public static extern cudaError_t cudaGraphicsSubResourceGetMappedArray(out cudaArray_t array,
                IntPtr resource, uint arrayIndex, uint mipLevel);
